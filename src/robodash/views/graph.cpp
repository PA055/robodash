...
void rd::Graph::plot(std::string series, double value) { buffers[series] = value; }

// ============================== Updater Task ============================== //

void rd::Graph::update_task() {
	while (true) {
		for (auto const &[series_name, last_value] : buffers) {
			...
			lv_chart_set_next_value(chart, this_series, last_value);
		}
